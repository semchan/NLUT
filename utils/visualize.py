...
if __name__ == "__main__":
    ...
    path_test = "/home/chenys/surface/NLUT_NET/finetuning_train/checkpoint/19_finetuning_style_lut.pth"
    model = torch.load((path_test),map_location=torch.device('cpu'))
    luts_model = model['state_dict']["CLUTs.LUTs"]

    # luts = cube_to_lut(luts_model.reshape(-1,3,33,33,33))
    ...
    draw_strong(luts+identity3d_tensor(32).unsqueeze(0), save_dir="./") 
    draw_weak(luts+identity3d_tensor(32).unsqueeze(0), save_dir="./")
    ...
    # model = torch.load(os.path.join(root, name, "model{:0>4}.pth".format(epoch)),map_location=torch.device('cpu'))
    model = torch.load(path_test,map_location=torch.device('cpu'))
    matrix_weak = model['state_dict']['CLUTs.w_Layers'].numpy().reshape(-1,1,33,33)
    draw_matrix_weak(matrix_weak,save_dir="./LUT.png")